  ...
  return (
    <div className={`relative w-screen h-screen overflow-hidden bg-slate-950 ${className}`} style={style}>
      ...
      {hasRemote && (
        ...
        >
          {videoList.slice(0, MAX_ON_SCREEN).map((video, index) => (
            <div
              key={index}
              className="overflow-hidden rounded-md flex-shrink-0 bg-slate-900"
      ...
      {videoList.length > MAX_ON_SCREEN && (
        <div className="absolute bottom-0 left-0 right-0 h-20 flex items-center gap-2 px-3 overflow-x-auto bg-slate-950/80 backdrop-blur-sm">
          {videoList.slice(MAX_ON_SCREEN).map((video, index) => (
            <div
              key={index}
              className="h-full aspect-video flex-shrink-0 rounded-md overflow-hidden bg-slate-900"
            >
              {video}
            </div>
          ))}
