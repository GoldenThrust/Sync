...
export default function OneFormField({ type, name, placeholder, className = '', FormIcon, SubmitIcon, OnSubmit, onChange = () => { } }) {
    const {
        handleSubmit,
        register,
    } = useForm();
    ...
    return <form className={`h-12 flex justify-center ${className}`} onSubmit={handleSubmit(OnSubmit)}>
        <div className={`flex justify-center items-center text-field-style w-full rounded-lg text-sm outline-none rounded-e-none`}>
            {FormIcon}<input type={type} {...register(name)} placeholder={placeholder} onChange={onChange} className="w-5/6 p-2 h-5/6 bg-transparent outline-none text-gray-300" />
        </div>
