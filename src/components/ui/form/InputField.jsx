...
export default function InputField({ data, register }) {
    ...
    if (data.type === 'select') {
        ...
        return <label className="block relative" htmlFor={data.name}>
            <input type={data.type} {...register(data.name, { required: data.required })} className={`text-field-style rounded-lg p-3 w-full text-sm outline-none ${data.className || ''}`} defaultValue={data.value || ''} onBlur={onInputChange} />
            <div className="absolute top-1/2 -translate-y-1/2 left-2 pointer-events-none" ref={divRef} >{data.placeholder}</div>
        </label>
    }
}
